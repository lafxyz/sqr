...
public class QueueService
{
    ...
    public QueueService(DatabaseService dbService, Translator translator)
    {
        ...
        if (connectedGuild.Queue.Any() == false)
        {
            ...
            if (language.IsSlavicLanguage)
            {
                embed.WithDescription(
                    string.Format(
                        music.QueueWorkerTranslation.LeavingInNSeconds, seconds,
            ...
            {
                embed.WithDescription(string.Format(music.QueueWorkerTranslation.LeavingInNSeconds, seconds));
                await context.Channel.SendMessageAsync(embed);
            }
            ...
            if (connectedGuild.Queue.Any()) return;
            await DisconnectAsync(connection);
            embed.WithDescription(music.QueueWorkerTranslation.EmptyQueue);
            await context.Channel.SendMessageAsync(embed);
            return;
        }
        ...
        var finalEmbed = new DiscordEmbedBuilder()
            ...
            .WithDescription(string.Format(
                             ...
                             + $"{(connection.CurrentState.CurrentTrack!.SourceName == "spotify" ?
                                         music.QueueWorkerTranslation.IfPlaybackStopped : "")}");
        
        await context.Channel.SendMessageAsync(finalEmbed);
        await Task.Delay(1000);
